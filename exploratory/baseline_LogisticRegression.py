...
No GPU required. Runtime: seconds even on full 18056-sample dataset.

Usage (from project root):
    python "CHEAT_TEST/baseline_lr.py"
"""
...
OUTPUT_DIR  = "CHEAT_TEST/lr_results"
...
def evaluate(clf, X_test, y_test, test_sources, out_dir):
    ...
    out_csv = os.path.join(out_dir, "lr_predictions.csv")
    pd.DataFrame({
        "source":  test_sources,
        "label":   y_test,
        "pred":    preds,
...
def main():
    ...
    evaluate(lr, X_test, test_labels, test_sources, OUTPUT_DIR)
    print("\nDone.")
